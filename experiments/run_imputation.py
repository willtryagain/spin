...
def run_experiment(args):
    ...
    run = wandb.init(project="mtst v2", name="num_heads=1, t_s=24")
    ...
    transformed_op = scalers["data"].inverse_transform(output["y_hat"])

    output["y_hat"] = torch.where(output["mask"], transformed_op, output["y_hat"])
